...
function challengePayload(c){
  if(!c)return null;
  const d={...c.data};
  delete d.answer;delete d.answers;
...
function makeChallenge(r,p){
  ...
  if(type===1){
    const a=randomBetween(7,15), b=randomBetween(4,12), c1=randomBetween(3,9), answer=(a*b)+c1;
    c.data={a,b,c1,answer,prompt:`Kraak de sleutel: (${a} × ${b}) + ${c1} = ?`,task:'Je mag het antwoord alleen indienen zolang de timer loopt. De moeilijkheid is bewust hoger dan een simpele éénstaps-som.'};
  }
  if(type===2){
  ...
  if(type===3){
    c.data={target:other?.id||null,phrase:'DE LIJN IS VEILIG',prompt:'Jij en één andere speler moeten dezelfde geheime zin binnen 3 seconden bevestigen.',task:'Bel elkaar eerst. Wanneer jullie klaar zijn, drukken jullie zo dicht mogelijk tegelijk op de knop.'};
  }
  if(type===4){
...
function startGame(r){
  r.started=true;r.roles=[];r.challenge=null;r.eavesdropUsed=false;r.eavesdropper=null;r.eavesdropPending=null;r.eavesdropUntil=0;
  ...
  r.players.forEach(pl=>{
    ...
    send(pl.ws,{type:'gameStart',goal,role,codeMasked:'----',timerEndsAt:Date.now()+180000});
  ...
  r.players.forEach((pl,i)=>send(pl.ws,{type:'privateInfo',text:[
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(m.type==='join'){
      ...
      if(!r){r={code,players:[],host:p.id,started:false,roles:[],call:null,eavesdropUsed:false,eavesdropper:null,eavesdropPending:null,challenge:null,challengeTimer:null,eavesdropTimer:null};rooms.set(code,r)}
      ...
      send(ws,{type:'joined',room:code,id:p.id,host:r.host});send(ws,roomState(r));return;
    }
    ...
    if(m.type==='coChallengeAnswer'){
      ...
      if(c.syncClicks[c.player]&&c.syncClicks[c.data.target]&&Math.abs(c.syncClicks[c.player]-c.syncClicks[c.data.target])<=3000){
        c.completed=true;const a=playerById(r,c.player),b=playerById(r,c.data.target);[a,b].forEach(q=>{if(q){send(q.ws,{type:'challengeComplete',reward:'De geheime zin werd binnen 3 seconden bevestigd.'});sendClue(r,q,'Beloning: Wederzijdse Bekentenis');}});
      } return;
    }
    if(m.type==='challengeAnswer'){
      ...
      if(c.type===2){ok=Boolean(m.confirm);reason='Geheime frequentie gebruikt: één codecijfer is vrijgegeven.';}
    ...
    if(m.type==='eavesdropAnswer'){
      ...
      if(String(m.answer||'').trim()!==String(ep.answer)){r.eavesdropPending=null;return send(ws,{type:'challengeFailed',text:'Fout antwoord. De eenmalige afluisterkans is verloren.'});}
  ...
  ws.on('close',()=>{
    ...
    if(!r.players.length){if(r.challengeTimer)clearTimeout(r.challengeTimer);if(r.eavesdropTimer)clearTimeout(r.eavesdropTimer);rooms.delete(p.room);}else broadcast(r,roomState(r));
  });
});

// A special one-time eavesdrop challenge is offered at a random moment after the game starts.
setInterval(()=>{
  for(const r of rooms.values()){
    ...
    if(Math.random()<0.22){
      const p=candidates[Math.floor(Math.random()*candidates.length)];
      const a=randomBetween(6,12),b=randomBetween(4,9),c=randomBetween(2,8);const answer=a*b+c;
      r.eavesdropPending={player:p.id,endsAt:Date.now()+20000,answer};
      send(p.ws,{type:'eavesdropChallenge',endsAt:r.eavesdropPending.endsAt,question:`(${a} × ${b}) + ${c} = ?`});
    }
  }
},5000);
